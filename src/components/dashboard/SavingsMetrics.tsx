...
import { subscriptions, lowUsageAlerts } from "@/lib/mockData";
...
export function SavingsMetrics() {
  ...
  const potentialSavings = lowUsageAlerts.reduce((sum, s) => sum + s.monthlyCost, 0);
  ...
  const metrics = [
    {
      title: t("analytics.currentSavings"),
      value: formatCurrency(pausedSavings),
      subtitle: t("stat.fromPaused"),
      icon: PiggyBank,
      color: "text-success",
      bg: "bg-success/10",
    },
    {
      title: t("analytics.potentialSavings"),
      value: formatCurrency(potentialSavings),
      subtitle: t("analytics.wastedOnUnused"),
      icon: AlertTriangle,
      color: "text-warning",
      bg: "bg-warning/10",
    },
    {
      title: t("analytics.annualProjection"),
      value: formatCurrency(annualProjection),
      subtitle: t("analytics.projectedYearly"),
      icon: TrendingUp,
      color: "text-primary",
      bg: "bg-primary/10",
    },
